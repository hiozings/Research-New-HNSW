...
# def run_experiment(sizes, dim=128, dbpath='./rocksdb_data', opt_mode=False, dpi=200, n_search=20, M=16, ef_construction=200, measure_recall=False):
#     results = []
...
#         # 在 run_experiment 函数中，修改召回率测量部分：
...
#         # 获取所有向量用于计算真实结果（使用优化版本）
#         all_vectors = fetch_all_vectors_numpy(8081, N, hnsw)
...
#                     # 计算真实结果（排除查询向量自身）- 使用numpy优化版本
#                     vectors_for_gt = {vid: vec for vid, vec in all_vectors.items() if vid != query_id}
#                     ground_truth = get_ground_truth_numpy(query_vector, vectors_for_gt, k=10)
...
#         results.append({'N': N, 'avg_rss': avg_mem, 'peak_rss': peak_mem, 'avg_recall': avg_recall ,'recall_results': recall_results if measure_recall else []})
...
#     # 在 calculate_memory_reduction 函数中添加：
#     recall_differences = []
...
#         print(f"[REDUCTION] N={N} 平均内存下降: {avg_reduction:.1f}%  峰值内存下降: {peak_reduction:.1f}%  召回率差异: {recall_diff:+.4f}")
...
def calculate_memory_reduction(baseline_results, optimized_results, dpi=200):
    ...
    with open('res/memory_reduction.json', 'w') as f:
        json.dump(reduction_results, f, indent=2)
    print(f"\n对比分析完成，结果保存到 res/memory_reduction.json")
...
if __name__ == '__main__':
    ...
    # 非优化模式
    baseline_results = run_experiment(args.sizes, args.dim, opt_mode=False, dpi=args.dpi, n_search=args.n_search, M=args.M, ef_construction=args.ef_construction, measure_recall=args.recall)
    # 优化模式
    if args.opt:
       optimized_results = run_experiment(args.sizes, args.dim, opt_mode=True, dpi=args.dpi, n_search=args.n_search, M=args.M, ef_construction=args.ef_construction, measure_recall=args.recall)
